...
/*   Updated: 2026/08/28 16:24:31 by elerazo-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static std::string validate_arg(std::string arg)
{
	...
	std::cout << "\033[31m ERROR \033[0m Is Forbidden negative numbers" << std::endl;
	exit(1);
    }
    ...
    //    return "Non-number arguments not allowed";
    	  std::cout << "\033[31m ERROR \033[0m Is Forbidden Non-number arguments" << std::endl;
	  exit(1);
    }
    if (nbr > INT_MAX || errno == ERANGE)
    {
    //    return "Too big arguments are not allowed";
	  std::cout << "\033[31m ERROR \033[0m Is Forbidden too big arguments" << std::endl;
	  exit(1);
    }
    return "";
}
...
static std::string validate(int argc, char** argv)
{
	...
	std::cout << "\033[31m ERROR \033[0m You need arguments" << std::endl;
    	exit(1);}
